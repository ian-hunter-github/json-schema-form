...
test -f packages/jsf-core/src/engine.ts || { err "Run this from json-schema-form/"; exit 1; }

log "Patching Ajv to allow vendor keywords (x-enumNames, x-enum-labels, discriminator)"
cp packages/jsf-core/src/engine.ts packages/jsf-core/src/engine.ts.bak

cat > packages/jsf-core/src/engine.ts <<'TS'
...
echo
ok "Patch complete. Reload the SPA:"
echo "  open examples/spa/index.html   (hard refresh with ⌘+Shift+R if cached)"
