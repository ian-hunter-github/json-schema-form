...
[[ -f "$FILE" ]] || { fail "Cannot find $FILE (run from repo root)"; exit 1; }

cp "$FILE" "$FILE.bak-remount"
info "Backed up → $FILE.bak-remount"
...
ok "Patched $FILE"
echo "[HINT] Now rebuild the package:"
echo "  npm --workspace @totnesdev/jsf-react run build"
